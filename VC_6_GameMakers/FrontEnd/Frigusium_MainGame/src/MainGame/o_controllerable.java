...
public class o_controllerable extends GameObject
{
	...
	public o_controllerable(double x, double y, String playerName)
	{
		...
		own_gun = (o_gun) Game.createGameObject(new o_gun(this,shooting_distanceX,shooting_distanceY));
	}
	...
	{
		// Drawing the Armor
		int sprite_x = (int)x - own_sprite.getWidth()/2;
		int sprite_y = (int)y - own_sprite.getHeight() + 3;
		g.drawImage(own_sprite.getImage((int)ani_imageIndex),sprite_x,sprite_y,null);
	...
	private void AI()
	{
		if ( this != Game.PLAYER && Game.GameSessionRunning) 
		{
	...
	public void movementControl()
	{
		...
		double ix = 0; double addx = spd_x/10;
		while (Math.abs(ix) < Math.abs(spd_x))
		{
			if (functions.terrainCollisionCheck((int)(x + ix + addx),(int)y))
			{
				spd_right = 0;
				spd_left = 0;
				break;
		...
		double iy = 0; double addy = spd_y/10;
		while ( Math.abs(iy) < Math.abs(spd_y))
		{
			if ( functions.terrainCollisionCheck((int)x,(int)(y + iy + addy)) )
			{
				spd_up = 0;
				spd_down = 0;
				break;
	...
	public void addKillCount() { stat_killCount++; }
	public int getKillCount() { return stat_killCount++; }
	
	public void destroy()
	{
		for ( int i = 0; i < 30; i++ ) { Game.createGameObject(new o_particle_ember(x,y)); }
		
		if ( lastHitter == Game.PLAYER ) { Game.UI.addPlayerText(getPlayerName() + " Silenced"); }
		if ( lastHitter != null ) { ((o_controllerable)lastHitter).addKillCount(); }
		
		if ( this == Game.PLAYER )
		{
			setBlood(0);
			if ( Game.getPlayerNum() > 1) Game.STAT_KILLEDBY = ((o_controllerable) lastHitter).getPlayerName();
			Game.STAT_KILLCOUNT = this.getKillCount();
			Game.playerNum = 0;
			ArrayList<GameObject> PlayerList = Game.gameClient.getPlayerList();
			for ( int i = 0; i < PlayerList.size(); i++ )
			{
				if ( PlayerList.get(i).getBlood() > 0 ) { Game.playerNum++; }
			}
			Game.STAT_RANK = Game.playerNum;
