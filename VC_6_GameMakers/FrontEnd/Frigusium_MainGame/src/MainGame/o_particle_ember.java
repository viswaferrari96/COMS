...
public class o_particle_ember extends GameObject
{
	...
	private double spd = r.nextInt(140)/100. + 0.06;
	private double angle = r.nextInt(360);
	private double angleChange = r.nextInt(20)-10;
	...
	private double initialSpd_x = 0;
	private double initialSpd_y = 0;
	...
	private void move()
	{
		spd_x = Math.cos(Math.toRadians(angle))*spd + correctX + initialSpd_x;
		spd_y =-Math.sin(Math.toRadians(angle))*spd + correctY + initialSpd_y;
		
		angle += angleChange;
		
		angleChange = functions.clamp(angleChange+((r.nextInt(100)-50)/100.),-8,8);
		
		x += spd_x;
		y += spd_y;
	}
	
	public void setInitialSpd_x ( double initialSpd_x ) { this.initialSpd_x = initialSpd_x; }
	public void setInitialSpd_y ( double initialSpd_y ) { this.initialSpd_y = initialSpd_y; }
}
